...
in the backup/ folder.  Files are written into status-named subfolders:
...
e.g.  backup/20_mar_2026_backup.zip

Replaces the former raw-sqlar backup.  Can be run standalone or called from the master launcher.
"""
...
def _build_staging_path_map(build_manifest_path: str) -> dict[str, str]:
    ...
    (e.g. np2kai/bios.rom vs keropi/bios.rom) don't collide in the backup zip or
...
def run(config: configparser.ConfigParser, base_dir: str = ".") -> bool:
    ...
    while True:
        ...
        zip_name = f"{stamp}_backup{suffix}.zip"
    ...
    print(f"[backup] Build manifest: {build_manifest_path!r}")
    print(f"[backup] Dest          : {zip_path!r}")

    # Load staging path map for unverifiable / mismatch files
    staging_map = _build_staging_path_map(build_manifest_path)
    if staging_map:
        print(f"[backup] Loaded {len(staging_map)} staging paths from build manifest.")
    else:
        print("[backup] WARNING: build manifest not found or empty — "
    ...
    try:
        ...
        print(f"[backup] ERROR: could not read database: {exc}")
        conn.close()
        return False
    # Load alias mappings before closing — needed for the sidecar.
    try:
        alias_rows = conn.execute(
            "SELECT canonical_name, sqlar_name FROM canonical_aliases ORDER BY canonical_name"
        ).fetchall()
    ...
    if not file_rows:
        print("[backup] WARNING: database is empty — nothing to back up.")
        return True
    ...
    try:
        with zipfile.ZipFile(zip_path, "w", compression=zipfile.ZIP_STORED) as zf:
            ...
            # Write blob_map.json so bios_restore.py can map every blob back to
            # its canonical name without relying on the current manifest.  This
            # is what allows orphaned blobs (whose canonical left the manifest)
            # to be identified and saved rather than silently dropped.
            blob_map = {
                sqlar_name: {"canonical_name": canonical_name, "status": status}
                for sqlar_name, _data, canonical_name, status in file_rows
            }
            zf.writestr(".blob_map.json", json.dumps(blob_map, indent=2))
            print(f"[backup] Wrote {len(blob_map)} blob mapping(s) to .blob_map.json sidecar.")

            # Write canonical_aliases sidecar so bios_restore.py can
            # restore alias relationships for canonicals with no declared MD5.
            if alias_rows:
                sidecar = [{"canonical_name": c, "sqlar_name": s} for c, s in alias_rows]
                zf.writestr(".aliases.json", json.dumps(sidecar, indent=2))
                print(f"[backup] Wrote {len(alias_rows)} alias mapping(s) to .aliases.json sidecar.")

    except OSError as exc:
        print(f"[backup] ERROR: could not write zip: {exc}")
        return False
